...
class IngestAgent:
    ...
    async def _update_overview(self) -> None:
        ...
        resp = await self._provider.complete(
            messages=[Message(role="user",
                              content=self._language_directive() +
            _OVERVIEW_PROMPT.format(pages=pages_str))],
            temperature=0.3,
            max_tokens=512,
        )
    ...
    def _language(self) -> str:
        return self._cfg.wiki.language.strip() if self._cfg else ""
    ...
    def _language_directive(self) -> str:
        ...
        if not lang:
            return ""
        return (
            f"OUTPUT LANGUAGE: Write ALL synthesized prose — page_content, "
            f"update_content, titles, and section headings — in {lang}, even when "
            f"the source document is in another language. Translate faithfully; keep "
            f"technical terms and proper nouns accurate. Keep slugs, [[wikilinks]], "
            f"and ^[citation] markers exactly as specified (Latin-alphabet slugs, "
            f"unchanged filenames/line ranges).\n\n"
        )
    ...
    async def ingest(self, source: str, force: bool = False, bust_cache: bool = False) -> IngestResult:
        ...
        decision_prompt = self._language_directive() + decision_prompt
